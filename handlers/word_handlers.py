...
async def handle_training_options(message: types.Message, state: FSMContext):
    ...
    words_to_learn = random.sample(WORDS, k=num_words)
...
async def show_next_word(message: types.Message, state: FSMContext):
    ...
    await message.answer(f'Переведите слово: {current_word}', reply_markup=make_answer_keyboard(current_translation))
